var AudioCallBack = {
	beforeShowAd: () => {
		...
		AudioCallBack.muted = WebAudioEngine.muted;
		WebAudioEngine.muted = true;
	...
	afterShowAd: () => {
		...
		WebAudioEngine.muted = AudioCallBack.muted;
	}
};
//*-------- Show Interstitial --------*//
	...
	ysdk.adv.showRewardedVideo({
		callbacks: {
			...
			onError: (e) => {
				...
				if(failure)failure();
				if(window.isRussian)
					Prompt("Реклама в данный момент недоступна, пожалуйста, попробуйте еще раз позже!");
				else
					Prompt("Ads are currently unavailable, please try again later!");
			}
		}
	})
}
